...
-----------------------------------------------------------------------------------------------------------------------------------------
vCLIENT = {}
Tunnel.bindInterface("arcade-atm",vCLIENT)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local machineGlobal = 600 -- Tempo para poder assaltar um ATM novamente
local machineStart = false
local idgens = Tools.newIDGenerator()
local blips = {}

local policiaisMin = -1 -- Numero minimo de policiais em serviço para poder assaltar
local pagamentoMin = 15000 -- Valor minimo que recebe ao assaltar o ATM
local pagamentoMax = 25000 -- Valor maximo que recebe ao assaltar o ATM
local tempoRoubo = 600 -- Tempo para poder assaltar um ATM novamente
local recompensa = "dinheirosujo" -- Item dropado quando o ATM for explodido
-----------------------------------------------------------------------------------------------------------------------------------------
-- VERIFICAÇÃO PARA O INICIO DO ROUBO
-----------------------------------------------------------------------------------------------------------------------------------------
function vCLIENT.startMachine()
	local source = source
	...
	for l,w in pairs(policia) do
		...
		if player then
			async(function()
				...
				blips[ids] = vRPC.addBlip(player,x,y,z,1,59,"Roubo de ATM em andamento",0.5,true)
				TriggerClientEvent('chatMessage',player,"190",{64,64,255},"O roubo começou em um ^1ATM^0, dirija-se até o local e intercepte o assaltante.")
...
-----------------------------------------------------------------------------------------------------------------------------------------
function vCLIENT.stopMachine(x,y,z)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
